...
class HelpDropdown(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if val == "ai":
            ...
            embed.add_field(name="Economy Controls", value=f"`{p}addcoins @user [amt]` - Give coins\n`{p}takecoins @user [amt]` - Take coins\n`{p}airdrop [reason] [amount]` - Reward all known users + DM them\n`{p}grantvip @user [days]` - Give VIP", inline=False)
            embed.add_field(name="AI / TTS", value=f"`{p}ttstoggle [on/off]` - Enable or disable MiMo TTS", inline=False)
            embed.add_field(name="Backups", value=f"`{p}dbbackup` - Upload economy.db to WebDAV now\n`{p}dbbackupauto [on/off]` - Toggle automatic WebDAV backup\n`{p}dbbackupinterval [minutes]` - Set backup interval\n`{p}dbbackupstatus` - Show backup status\n`{p}dbrestore [latest|filename] confirm [restart]` - Restore economy.db from WebDAV\n`{p}restartbot confirm` - Restart the bot via systemd", inline=False)
